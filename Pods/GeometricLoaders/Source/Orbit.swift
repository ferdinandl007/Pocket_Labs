...
public class Orbit: GeometricLoader {
    ...
    internal override func configureLoader() {
        if orbitRadius == nil {
            orbitRadius = 3 * loaderView.frame.width / 4
        } else {
            assert(orbitRadius! < (loaderSuperview?.frame.width)! / 2, "Radius \(String(describing: orbitRadius)) has to less or equals than self.loaderSuperview?.frame.width/2 \((loaderSuperview?.frame.width)! / 2)")
        }
