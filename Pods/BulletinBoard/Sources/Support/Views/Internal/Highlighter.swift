...
class Highlighter {
    ...
    func handleTouchesBegan(_: Set<UITouch>, with _: UIEvent?) {
        target?.highlight()
    }
    ...
    func handleTouchesMoved(_ touches: Set<UITouch>, with _: UIEvent?) {
    ...
    func handleTouchesEnded(_ touches: Set<UITouch>, with _: UIEvent?) {
        guard let mainTouch = touches.first else {
            return
        }
